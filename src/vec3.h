...
class vec3 {
	public:
		...
		// generates arbitary random vectors
		static vec3 random() {
...
inline vec3 random_unit_vector() {
	while (true) {
		...
		if (1e-160 < lensq && lensq <= 1) { // reject until a sample with a point inside the sphere is generated `111
			return p / sqrt(lensq);
		}
	}
}
...
inline vec3 random_on_hemisphere(const vec3& normal) {
	...
	if (dot(on_unit_sphere, normal) > 0.0) { // vector is in correct/same hemisphere as normal 
